...
public class SignAuthFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		if (!StringUtils.hasText(sign)) {
			...
			ResultUtil.writeJavaScript(resp, ErrorCodeEnum.PARAMS, "非法请求:缺少签名信息");
			return;
		}
		try {
			...
			if ((System.currentTimeMillis() - signTime) > encryptProperties.getSignExpireTime() * signExpireTime) {
				log.info("非法请求:已过期");
				ResultUtil.writeJavaScript(resp, ErrorCodeEnum.PARAMS, "非法请求:已过期");
				return;
			}
				...
				for (String key : paramsSet) {
					if (!"signTime".equals(key)) {
						...
						if (!signValue.equals(reqValue)) {
							log.info("非法请求:参数被篡改");
							ResultUtil.writeJavaScript(resp, ErrorCodeEnum.PARAMS, "非法请求:参数被篡改");
							return;
						}
					}
				}
			...
			ResultUtil.writeJavaScript(resp, ErrorCodeEnum.PARAMS, "非法请求:" + e.getMessage());
			return;
		}
